...
-- 유저랭킹조회
SELECT PNT_ID, PNT_SCORE,
( SELECT COUNT(*)+1 FROM POINT WHERE PNT_SCORE > ( SELECT PNT_SCORE FROM POINT WHERE PNT_ID = 'TEST1' ) ) AS PNT_RANK
FROM POINT WHERE PNT_ID = 'TEST1';

-- 1~3위 유저정보조회
SELECT PNT_ID, PNT_SCORE, ( SELECT COUNT(*)+1 FROM POINT WHERE PNT_SCORE > T.PNT_SCORE ) AS PNT_RANK,
MBR_ID, MBR_NICK, MBR_PROFILE FROM POINT AS T
JOIN MEMBER ON T.PNT_ID = MEMBER.MBR_ID ORDER BY PNT_RANK LIMIT 0,3

-- 파일업로드
INSERT INTO UPLOAD ( ULD_NO, ULD_FNO, ULD_ID, ULD_TYPE, ULD_TEXT, ULD_UUID, ULD_FILENAME, ULD_DATE )
VALUES (1, 1, 'TEST1', 'COMMUNITY', 'NORMAL', 'ce53bd4f-9d1f-45be-b57d-1c6b4b139a75', 'TEST1.PNG', NOW());